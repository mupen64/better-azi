...
bool WASAPISoundDriver::ClassRegistered = WASAPISoundDriver::ValidateDriver() ? SoundDriverFactory::RegisterSoundDriver(SND_DRIVER_WASAPI, WASAPISoundDriver::CreateSoundDriver, "WASAPI Driver (experimental)", 0) : false;
...
DWORD WINAPI WASAPISoundDriver::AudioThreadProc(LPVOID lpParameter)
{
    ...
    hr = pAudioClient->GetMixFormat((WAVEFORMATEX**)&pwfx);
    EXIT_ON_ERROR(hr);
    ...
    EXIT_ON_ERROR(hr); // Start playing.
    ...
    EXIT_ON_ERROR(hr); // Stop playing.

    // Graceful exit
    goto NiceExit;
Exit:
    DEBUG_OUTPUT("WASAPI: Error on thread\n");
    MessageBox(NULL, "An error occurred in the WASAPI audio thread.  This will result in no audio or a hang if prevent buffer overflow is checked.  As this is an experimental audio code, please submit a bug report to github.  Thank you!", "AziAudio ERROR", MB_OK);
NiceExit:
    CoTaskMemFree(pwfx);
