...
bool DirectSoundDriverLegacy::ClassRegistered = DirectSoundDriverLegacy::ValidateDriver() ? SoundDriverFactory::RegisterSoundDriver(SND_DRIVER_DS8L, DirectSoundDriverLegacy::CreateSoundDriver, "DirectSound 8 Legacy Driver", 5) : false;
...
void DirectSoundDriverLegacy::FillBuffer(BYTE* buff, DWORD len)
{
    ...
    {
        while ((remainingBytes > 0) && (cnt != len))
        { // Optimize this copy routine later
        ...
        { // Cycle around until a new buffer position is available
            ...
            __try // PJ64 likes to close objects before it shuts down the DLLs completely...
            {
                ...
                if (play_pos > last_play_pos)
                    bytesMoved = play_pos - last_play_pos;
                else
                    bytesMoved = TOTAL_SIZE - last_play_pos + play_pos;
...
u32 DirectSoundDriverLegacy::AddBuffer(u8* start, u32 length)
{
    ...
    {
        while ((remainingBytes + length) > MAXBUFFER)
        { // Halve the buffer...
            Sleep(1);
        }
    }
    ...
    if (Configuration::getForceSync())
    {
        ...
        while (remainingBytes > LOCK_SIZE * 2)
        { // Force buffer sync
            Sleep(1);
        }
    }
...
u32 DirectSoundDriverLegacy::GetReadStatus()
{
    ...
    if (Configuration::getAIEmulation() == true)
    {
        ...
        { // This was the problem with SP_DMA_READ Error
            if (lastLength == 0)
                return 0;
            if (remainingBytes > lastLength)
                return (remainingBytes % lastLength) & ~0x3;
            else
                return remainingBytes & ~0x3;
            // return 0;//remainingBytes;
        }
#endif
    }
